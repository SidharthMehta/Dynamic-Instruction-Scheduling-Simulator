...
#define ADDRESS_SIZE 32 //Size of address bus.
#define vacant -1
using namespace std;
...
CACHE::CACHE(unsigned long BLOCKSIZE, unsigned long CACHE_SIZE, unsigned long CACHE_ASSOC, unsigned long DATA_BLOCKS, unsigned long ADDR_TAGS, CACHE *nextLevel, unsigned long HIT_LATENCY, unsigned long MISS_LATENCY)
{
...
void CACHE::CacheStatus()
{
    for (unsigned long i = 0; i < SETS; i++)
    {
        ...
        for (unsigned long j = 0; j < CACHE_ASSOC; j++)
        {
            if (tagStore[i][j].TAG[0] == vacant)
            {
                cout << "\t0 ";
            }
            else
            {
                cout << hex << tagStore[i][j].TAG[0] << " ";
            }
        }
    }
}

unsigned CACHE::readFromAddress(unsigned long address)
{
    unsigned latency = HIT_LATENCY;
    READ++;
    //Calculation of bits
        ...
        for (unsigned long j = 0; j < CACHE_ASSOC; j++)
        {
            //if (tagStore[index][j].frequency == k)
            {
                if (tagStore[index][j].TAG[addrTAG] == TAG && tagStore[index][j].dataStore.isValid[sector] == false)
                {
                    ...
                    latency = requestReadFromNext(address);
...
unsigned CACHE::requestReadFromNext(unsigned long address)
{
    if (nextLevel != NULL)
    {
        return nextLevel->readFromAddress(address);
    }
    else
    {
        return MISS_LATENCY;
