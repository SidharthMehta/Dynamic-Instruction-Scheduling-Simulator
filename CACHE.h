...
private:
    ...
    unsigned requestReadFromNext(unsigned long);
...
public:
    ...
    unsigned long HIT_LATENCY;
    unsigned long MISS_LATENCY;
    float MISS_RATE = 0;
    ...
    //Constructor
    CACHE(unsigned long, unsigned long, unsigned long, unsigned long, unsigned long, CACHE *, unsigned long, unsigned long);

    //Display contents of CACHE
    void CacheStatus();

    //Read from CACHE
    unsigned readFromAddress(unsigned long);
